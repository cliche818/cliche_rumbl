defmodule ClicheRumbl.VideoChannel do
  ...
  alias ClicheRumbl.AnnotationView

  def join("videos:" <> video_id, params, socket) do
    last_seen_id = params["last_seen_id"] || 0
    video_id = String.to_integer(video_id)
    video = Repo.get!(ClicheRumbl.Video, video_id)
  ...
  def handle_in(event, params, socket) do
    user = Repo.get(ClicheRumbl.User, socket.assigns.user_id)
  ...
  def handle_in("new_annotation", params, user, socket) do
    changeset =
      ...
      |> ClicheRumbl.Annotation.changeset(params)
  ...
  defp compute_additional_info(ann, socket) do
    for result <- ClicheRumbl.InfoSys.compute(ann.body, limit: 1, timeout: 10_000) do
      attrs = %{url: result.url, body: result.text, at: ann.at}
      info_changeset =
        Repo.get_by!(ClicheRumbl.User, username: result.backend)
        |> build_assoc(:annotations, video_id: ann.video_id)
        |> ClicheRumbl.Annotation.changeset(attrs)
