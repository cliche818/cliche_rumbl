defmodule ClicheRumbl.AuthTest do
  use ClicheRumbl.ConnCase
  alias ClicheRumbl.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(ClicheRumbl.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %ClicheRumbl.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%ClicheRumbl.User{id: 123})
      |> send_resp(:ok, "")
